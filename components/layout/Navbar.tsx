...
import { Compass, Menu, X, LogIn, User, Brain } from "lucide-react";
    ...
    <>
      <motion.header className="fixed top-0 inset-x-0 z-50">
        ...
        <nav className="relative max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          {/* Mobile burger */}
          <button className="md:hidden text-stone-mist hover:text-foreground transition-colors" onClick={() => setOpen(!open)}>
        ...
        >
          {LINKS.map(({ href, label }) => (
            <Link key={href} href={href} onClick={() => setOpen(false)}>
              <div className="px-4 py-3 rounded-lg text-sm font-heading font-medium text-stone-mist hover:text-terra-light transition-colors">
                {label}
              </div>
            </Link>
          ))}
          <Link href={user ? "/dashboard" : "/auth"} onClick={() => setOpen(false)}>
            <div className="mt-2 px-4 py-3 rounded-lg text-sm font-heading font-bold btn-terra text-center">
