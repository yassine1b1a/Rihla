    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          {site.name_ar && <span className="text-arabic text-sm text-sand-DEFAULT opacity-70">{site.name_ar}</span>}
        </div>
        {site.unesco && (
          <span className="text-xs px-2 py-0.5 rounded-full font-mono flex-shrink-0 ml-2"
      ...
      <div className="flex flex-wrap gap-1.5 mt-3">
        {site.tags.slice(0, 3).map(t => (
          <span key={t} className="text-xs px-2 py-0.5 rounded font-mono"
            style={{ background: "rgba(255,255,255,0.05)", color: "#7A6E62", border: "1px solid rgba(255,255,255,0.06)" }}>{t}</span>
        ))}
      </div>
    </motion.div>
  );
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              {result.unesco && (
                <span className="text-xs px-2 py-0.5 rounded-full font-mono"
                  style={{ background: "rgba(232,201,138,0.15)", color: "#E8C98A", border: "1px solid rgba(232,201,138,0.25)" }}>UNESCO</span>
              )}
            </div>
            <div className="flex flex-wrap items-center gap-3 text-sm text-stone-mist">
              <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-terra-light" />{result.city || result.country}</span>
              <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-terra-light" />{result.period}</span>
        ...
        <div className="p-5 rounded-2xl" style={{ background: "rgba(28,35,48,0.6)", border: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <div className="flex flex-wrap gap-2">
            {result.nearby_sites.map((s: string, i: number) => (
              <span key={i} className="text-xs px-3 py-1.5 rounded-full font-heading font-medium"
...
export default function HeritagePage() {
  ...
  const identify = async () => {
    if (!input.trim()) {
      setError("Please enter a description or image URL");
      return;
    }
    ...
    try {
      console.log("Sending heritage recognition request:", {
        type: mode === "url" ? "image_url" : "description",
        value: input,
      ...
      if (!res.ok) {
        throw new Error(data.error || data.details || "Recognition failed");
      }
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 pt-24 pb-16">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          ...
          <p className="text-stone-mist text-lg max-w-xl mx-auto">
            Describe what you see or paste an image URL to instantly unlock the history, legends and secrets of any heritage site.
          </p>
        </motion.div>
            ...
            <div className="p-6 rounded-2xl" style={{ background: "rgba(28,35,48,0.8)", border: "1px solid rgba(255,255,255,0.06)" }}>
              ...
              <div className="flex gap-1 p-1 rounded-xl mb-5" style={{ background: "#0F1419" }}>
                {(["describe", "url"] as Mode[]).map(m => (
                  <button 
                    ...
                    {m === "describe" ? "Describe it" : "Image URL"}
                  </button>
                ))}
              </div>
              ...
              <div className="mb-4">
                <label className="text-xs font-mono text-stone-mist uppercase tracking-widest mb-2 block">
                  {mode === "describe" ? "Describe the site" : "Paste image URL"}
                </label>
                {mode === "describe" ? (
                  <textarea 
                    value={input} 
              ...
              {/* Country hint */}
              <div className="mb-4">
                <label className="text-xs font-mono text-stone-mist uppercase tracking-widest mb-2 block">Country hint</label>
                <select 
                  value={country} 
              ...
              {error && (
                <div className="mb-4 p-3 rounded-xl text-xs flex items-start gap-2 text-red-400"
              ...
              <motion.button 
                ...
                style={{ 
                  boxShadow: "0 6px 24px rgba(26,122,110,0.3)", 
