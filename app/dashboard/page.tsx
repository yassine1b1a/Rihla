...
import { Compass, Sparkles, Camera, Leaf, Brain, ArrowRight, MapPin, LogOut } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const TOOLS = [
    { href: "/itinerary",      icon: Sparkles, label: t('nav.itinerary'),      desc: t('features.itinerary.desc'),     color: "#C84B31" },
    { href: "/explore",        icon: Brain,    label: t('nav.explore'),         desc: t('features.guide.desc'),         color: "#E8C98A" },
    { href: "/heritage",       icon: Camera,   label: t('nav.heritage'),        desc: t('features.heritage.desc'),      color: "#1A7A6E" },
    { href: "/sustainability",  icon: Leaf,    label: t('nav.sustainability'),   desc: t('features.sustainability.desc'), color: "#4ADE80" },
  ];
    ...
    <div className="min-h-screen bg-[#0F1419]" dir={dir}>
      ...
      <header className="relative z-10 border-b px-6 h-16 flex items-center justify-between"
        ...
        <div className="flex items-center gap-3" style={{ flexDirection: dir === 'rtl' ? 'row-reverse' : 'row' }}>
          ...
          <button onClick={signOut} title={t('dashboard.signOut')}
            className="p-2 rounded-lg text-stone-mist hover:text-foreground transition-colors hover:bg-white/5">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 py-12">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          ...
          <p className="text-stone-mist ml-14">{t('dashboard.planTrip')}</p>
        </motion.div>
                ...
                <Link href={tool.href}>
                  <motion.div whileHover={{ y: -3, borderColor: `${tool.color}35` }} whileTap={{ scale: 0.98 }}
                    ...
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <ArrowRight className={`w-4 h-4 text-stone-mist opacity-0 group-hover:opacity-100 transition-opacity group-hover:text-terra-light ${dir === 'rtl' ? 'rotate-180' : ''}`} />
                    </div>
